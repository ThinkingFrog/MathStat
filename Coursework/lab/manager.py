...
class SignalManager:
    ...
    def read_signal(self, dirpath: Path, signal_part: int) -> np.array:
        for idx, data_file in enumerate(dirpath.iterdir()):
            if idx == signal_part:
                return self.read_file(data_file)

    def read_file(self, filepath: Path) -> np.array:
    ...
    def read_dir(self, dirpath: Path) -> np.array:
        data = np.zeros(
            (len(dirpath.glob("*.txt")), self._signal_length, self._subarray_size)
        )
        for idx, data_file in enumerate(dirpath.glob("*.txt")):
            data[idx] = self.read_file(data_file)
        return data

    def get_average(self, data_array: np.array) -> np.ndarray:
