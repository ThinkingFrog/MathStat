""" Copied from https://github.com/MaximSmolskiy/tolsolvty and auto-reformatted with black+isort
"""
...
def tolsolvty(infA, supA, infb, supb, *varargin):
    ...
    if mi == ms:  # m - количество уравнений в системе
        ...
        raise ValueError(
            "Количество строк в матрицах левых и правых концов неодинаково"
        )
    if ni == ns:
        n = ns  # n - количество неизвестных переменных в системе
    else:
        raise ValueError(
            "Количество столбцов в матрицах левых и правых концов неодинаково"
        )
    ...
    if ki == ks:
        ...
        raise ValueError(
            "Количество компонент у векторов левых и правых концов неодинаково"
        )
    if k != m:
        raise ValueError(
            "Размеры матрицы системы не соответствуют размерам правой части"
        )
    ...
    if not all(infb <= supb):
        raise ValueError(
            "В векторе правой части задана неправильная интервальная компонента"
        )
    ...
    if nargin >= 5:
        ...
        if nargin >= 6:
            ...
            if size(weight, 0) != m:
                raise ValueError(
                    "Размер вектора весовых коэффициентов задан некорректно"
                )
            if any(weight <= 0):
                raise ValueError(
                    " Вектор весовых коэффициентов должен быть положительным"
                )
            if nargin >= 7:
                epsf = varargin[2]
                if nargin >= 8:
    ...
    if minsv != 0 and maxsv / minsv < 1.0e12:
    ...
    disp(" ")
    if tolmax < 0.0 and abs(tolmax / epsx) < 10:
